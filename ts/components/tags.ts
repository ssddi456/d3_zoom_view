...
import { tag, Tag } from '../entities/tag';

interface TagsTabVM {
    tags: KnockoutObservableArray<Tag>;
    filterWord: KnockoutObservable<string>;
    clearFilter() : void;
    filtedTags: KnockoutComputed<Tag[]>;
    addTag(): void;
    copyTag(tag:Tag, idx: number): void;
        ...
        createViewModel: function (params, componentInfo) {
            const vm: TagsTabVM = {
                ...
                filtedTags: ko.pureComputed(function () {
                    ...
                    return tags.filter(function (tag: Tag) {
                        const name = tag.name();
                        const desc = tag.desc();
                ...
                copyTag: function (_tag: Tag, idx: number) {
                    const newTag = tag();
            ...
            if (params.tags && params.tags.subscribe) {
                params.tags.subscribe(function (newVal: Tag[]) {
                    vm.init(newVal);
                });
            }
